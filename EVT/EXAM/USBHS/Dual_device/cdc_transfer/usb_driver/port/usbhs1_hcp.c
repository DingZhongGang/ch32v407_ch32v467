...
/* @global */
usbh_handle_t usbhs1h_handle;
static usbh_xfer_unit_t *_current_xfer_unit;

usb_rst_e usbhs1_hch_init(void)
{
    memset(&usbhs1h_handle, 0, sizeof(usbhs1h_handle));

    usbhs1h_handle.open = _open;
    usbhs1h_handle.close = _close;
    usbhs1h_handle.start_transfer = _start_transfer;
    usbhs1h_handle.root_set_feature = _root_set_feature;
    usbhs1h_handle.root_get_status = _root_get_status;
...
{
    uint8_t int_flag = USBHS1H->INT_FLAG;

    if (int_flag & USBHS_UHIF_SOF_ACT)
    {
        usbhs1h_handle.tick++;
        usbhs1h_handle.microframe = (USBHS1H->FRAME >> 16) & 0x07;
    }

    if (int_flag & USBHS_UHIF_TRANSFER)
    {
        if (USBHS1H->PORT_STATUS & USBHS_UHIS_PORT_CONNECT)
        {
            if (_current_xfer_unit->token == USBH_PID_IN)
            {
                _current_xfer_unit->length = USBHS1H->RX_LEN;
            }
            _current_xfer_unit->pid = USBHS1H->INT_ST & 0x0F;
        ...
        {
            usbhs1h_handle.xfering = USB_FALSE;
        }
    }

    USBHS1H->INT_FLAG = int_flag;

    uint8_t port_change = USBHS1H->PORT_STATUS_CHG;
    uint16_t port_status = USBHS1H->PORT_STATUS;

    if (port_change & USBHS_UHIF_PORT_CONNECT)
    {
        if (port_status & USBHS_UHIS_PORT_CONNECT)
        {
            usbh_device_insert(&usbhs1h_handle, 0, 0);
        }
        else
        {
            usbh_device_remove(&usbhs1h_handle, 0, 0);
        }
    }

    if ((port_change & USBHS_UHIF_PORT_SUSP) && ((port_status & USBHS_UHIS_PORT_SUSP) == 0))
    {
        usbh_device_resume(&usbhs1h_handle, 0, 0);
    }

    USBHS1H->PORT_STATUS_CHG = port_change;
}
...
static usb_rst_e _open(void)
{
    ...
    USBHS1H->CFG = USBHS_RST_LINK | USBHS_UH_PHY_SUSPENDM;
    USBHS1H->PORT_CFG = USBHS_UH_PD_EN | USBHS_UH_HOST_EN;
    USBHS1H->FRAME = USBHS_UH_SOF_CNT_EN;
    USBHS1H->CFG = USBHS_UH_SOF_EN | USBHS_UD_DMA_EN | USBHS_UD_PHY_SUSPENDM;
    USBHS1H->INT_EN = USBHS_UHIE_SOF_ACT | USBHS_UHIE_TRANSFER;
    USBHS1H->PORT_INT_EN = USBHS_UHIE_PORT_SUSP | USBHS_UHIE_PORT_CONNECT;
...
static usb_rst_e _close(void)
{
    USBHS1H->CFG = USBHS_UD_RST_LINK | USBHS_UD_RST_SIE | USBHS_UD_CLR_ALL;
...
static usb_rst_e _start_transfer(void)
{
    _current_xfer_unit = usbhs1h_handle.xfer_list;
    _unit_transfer(_current_xfer_unit);
    return USB_RST_OK;
}
...
static usb_rst_e _root_set_feature(hub_feature_e feature)
{
    ...
    case HUB_FEATURE_PORT_SUSPEND:
        USBHS1H->PORT_CTRL |= USBHS_UH_SET_PORT_SUSP;
        break;

    case HUB_FEATURE_PORT_RESET:
        USBHS1H->PORT_CTRL |= USBHS_UH_SET_PORT_RESET;
        break;
...
static void _unit_transfer(usbh_xfer_unit_t *xfer)
{
    ...
    {
        USBHS1H->RX_MAX_LEN = xfer->length;
        USBHS1H->RX_DMA = (uint32_t)xfer->buffer;
        USBHS1H->CONTROL = USBHS_UH_HOST_ACTION | xfer->token | (xfer->endp << 4) | supplement;
    }
    else if (xfer->token == USBH_PID_OUT || xfer->token == USBH_PID_SETUP)
    {
        USBHS1H->TX_LEN = xfer->length;
        USBHS1H->TX_DMA = (uint32_t)xfer->buffer;
        USBHS1H->CONTROL = USBHS_UH_HOST_ACTION | xfer->token | (xfer->endp << 4) | (xfer->toggle << 8) | supplement;
    }
    else if (xfer->token == USBH_PID_PING)
    {
        USBHS1H->TX_LEN = 0;
        USBHS1H->TX_DMA = 0;
        USBHS1H->CONTROL = USBHS_UH_HOST_ACTION | xfer->token | (xfer->endp << 4);
    }
}
